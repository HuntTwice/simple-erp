<template>
  <div>
    <div class="search">
      <el-input placeholder="请输入客户名查询" style="width: 200px;margin-right: 10px"
                v-model="selectConditions.name"></el-input>
      <el-select v-model="selectConditions.status" placeholder="客户状态">
        <el-option
    ...
    <div class="operation">
      ...
      <el-upload style="display: inline-block;margin:auto 10px" :action="$baseUrl+'/customers/import'"
    ...
    <el-dialog title="客户" :visible.sync="formVisible" width="40%" :close-on-click-modal="false" destroy-on-close>
      <el-form :model="form" label-width="100px" style="padding-right: 50px" :rules="rules" ref="formRef">
        <el-form-item label="客户名" prop="name" >
          <el-input v-model="form.name" placeholder="客户名" :disabled="form.id"></el-input>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    async expBatch() {
      ...
      // try {
      //   const response = await fetch(this.$baseUrl + '/customers/export', {
      //     method: 'GET',
    ...
    async load() {
      let result = await customerList(this.selectConditions, this.pageNum, this.pageSize)
        ...
        if (valid) {
          try {
            if (this.form.id == null)
              await addCustomerService(this.form)
            else
              await updateCustomerService(this.form)
    ...
    delBatch() {   // 批量删除
      ...
      this.$confirm('您确定批量删除这些数据吗？', '确认删除', {type: "warning"}).then(async () => {
        let result = await deleteCustomerBatchService(this.ids)
